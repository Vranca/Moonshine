...
namespace moon 
{
	...
	public:
		...
		//
		double Raycast(const Vector2D<double>& origin, double fRayAngle, const moon::Map& map, char target);
	...
	public:
		...
		inline void DrawPoint(int x, int y, uint32_t argb)									    {	m_RendererNew->DrawPoint(x, y, argb);						}
		inline void DrawLine(int x1, int y1, int x2, int y2, uint32_t argb)	                    {	m_RendererNew->DrawLine(x1, y1, x2, y2, argb);				}
		inline void DrawRect(const Rect& r, uint32_t argb)										{	m_RendererNew->DrawRect(r, argb);							}
		inline void FillRect(const Rect& r, uint32_t argb)										{	m_RendererNew->FillRect(r, argb);							}
		inline void ClearRenderer()																{	m_RendererNew->Clear();										}
		inline void ClearRenderer(uint32_t argb)												{	m_RendererNew->Clear(argb);									}
		inline void FillVerticalGradientRect(const Rect& r, uint32_t argb0, uint32_t argb1)		{	m_RendererNew->FillVerticalGradientRect(r, argb0, argb1);	}
	};
}
